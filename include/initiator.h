...
struct Initiator : sc_module {
    ...
    SC_CTOR(Initiator)
        : socket("socket") // Construct and name socket
          ,
          request_in_progress(0), m_peq(this, &Initiator::peq_cb) {
    ...
    void thread_process() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            // Set all attributes except byte_enable_length and extensions
            // (unused)
            ...
            trans->set_streaming_width(
                4); // = data_length to indicate no streaming
            ...
            // Timing annotation models processing time of initiator prior to
            // call
            delay = sc_time(rand_ps(), SC_PS);

            fout << hex << adr << " " << name()
                 << " new, cmd=" << (cmd ? 'W' : 'R') << ", data=" << hex
                 << data[i % 16] << " at time " << sc_time_stamp() << endl;
            ...
            if (status == tlm::TLM_UPDATED) {
                ...
                // The completion of the transaction necessarily ends the
                // BEGIN_REQ phase
    ...
    void peq_cb(tlm::tlm_generic_payload &trans, const tlm::tlm_phase &phase) {
        ...
        if (phase == tlm::END_REQ)
            fout << hex << trans.get_address() << " " << name()
    ...
    void check_transaction(tlm::tlm_generic_payload &trans) {
        ...
        fout << hex << adr << " " << name()
             << " check, cmd=" << (cmd ? 'W' : 'R') << ", data=" << hex << *ptr
