...
struct Target : sc_module {
    ...
    SC_CTOR(Target)
        : socket("socket"), n_trans(0), response_in_progress(false),
          next_response_pending(0), end_req_pending(),
    ...
    void peq_cb(tlm::tlm_generic_payload &trans, const tlm::tlm_phase &phase) {
        ...
        case tlm::BEGIN_REQ:
            ...
            if (n_trans == 2)
                ...
                if (status == tlm::TLM_COMPLETED) // It is questionable whether
                                                  // this is valid
                    break;
            }
...
#ifdef DEBUG
            fout << hex << trans.get_address() << " " << name()
                 << " END_RESP at " << sc_time_stamp() << endl;
#endif
            ...
            if (phase == internal_ph) {
                ...
                if (cmd == tlm::TLM_READ_COMMAND) {
                    *reinterpret_cast<int *>(ptr) = rand();
                    fout << hex << adr << " " << name()
                         << " Execute READ, target = " << name()
                         << " data = " << *reinterpret_cast<int *>(ptr) << endl;
                } else if (cmd == tlm::TLM_WRITE_COMMAND)
                    fout << hex << adr << " " << name()
                         << " Execute WRITE, target = " << name()
                         << " data = " << *reinterpret_cast<int *>(ptr) << endl;
    ...
    tlm::tlm_sync_enum send_end_req(tlm::tlm_generic_payload &trans) {
        ...
        if (status == tlm::TLM_COMPLETED) {
            ...
            // (TLM_UPDATED cannot occur at this point in the base protocol, so
            // need not be checked)
            trans.release();
            return status;
        }
